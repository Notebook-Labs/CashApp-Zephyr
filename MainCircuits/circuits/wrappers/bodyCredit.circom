...
include "../dynamic/username.circom";
...
include "../fixedBody/fix10.circom";
include "../fixedBody/creditFix1.circom";
include "../fixedBody/creditFix2.circom";
include "../fixedBody/creditFix3.circom";


template BodyCredit(msgBytes) {
    ...
    var fix9Len = 387;
    var fix10Len = 1673;
    var creditFix1Len = 385;
    var creditFix2Len = 847;
    var creditFix3Len = 460;
    ...
    var fix9LenVar = 17;
    var fix10LenVar = 72;
    var creditFix1LenVar = 17;
    var creditFix2LenVar = 36;
    var creditFix3LenVar = 19;
    ...
    var credit1Range = fix8LenVar + creditFix1LenVar + amountRange + credit1Max - credit1Min;
    var credit2Range = creditFix2LenVar + credit1Range + credit2Max - credit2Min;
    var methodRange = creditFix3LenVar + fix9LenVar + credit2Range + methodMax - methodMin;
    ...
    component creditFix1Uncertainty = UncertaintyExtraction(amountRange + creditFix1LenVar, creditFix1Len + creditFix1LenVar, partMin + fix1Len + paymentMin + fix2Len + photoMin + fix3Len + nameMin + fix4Len + usernamePartMin + fix5Len + firstAmountMin + fix6Len + messageMin + fix7Len + amountMin + fix8Len, msgBytes);
    creditFix1Uncertainty.indicatorLen <== amountLen - amountMin + messageLen - messageMin + firstAmountLen - firstAmountMin + usernamePartLen - usernamePartMin + nameLen - nameMin + photoLen - photoMin + paymentLen - paymentMin + partLen - partMin + encodedLen * (fix1LenVar + fix2LenVar + fix3LenVar + fix4LenVar + fix5LenVar + fix6LenVar + fix7LenVar + fix8LenVar);
    for (var i = 0; i < msgBytes; i++) {
        creditFix1Uncertainty.in[i] <== in[i];
    }
    signal CreditFix1Array[creditFix1Len + creditFix1LenVar];
    for (var i = 0; i < creditFix1Len + creditFix1LenVar; i++) {
        CreditFix1Array[i] <== creditFix1Uncertainty.out[i];
    }

    component creditFix1Regex = CreditFix1Regex();
    creditFix1Regex.encoded <== encodedLen;
    for (var i = 0; i < creditFix1Len + creditFix1LenVar; i++) {
        creditFix1Regex.in[i] <== CreditFix1Array[i];
    }


    component credit1Uncertainty = UncertaintyExtraction(credit1Range, creditFix2Len + creditFix2LenVar, partMin + fix1Len + paymentMin + fix2Len + photoMin + fix3Len + nameMin + fix4Len + usernamePartMin + fix5Len + firstAmountMin + fix6Len + messageMin + fix7Len + amountMin + fix8Len + creditFix1Len + credit1Min, msgBytes);
    credit1Uncertainty.indicatorLen <== credit1Len - credit1Min + amountLen - amountMin + messageLen - messageMin + firstAmountLen - firstAmountMin + usernamePartLen - usernamePartMin + nameLen - nameMin + photoLen - photoMin + paymentLen - paymentMin + partLen - partMin + encodedLen * (fix1LenVar + fix2LenVar + fix3LenVar + fix4LenVar + fix5LenVar + fix6LenVar + fix7LenVar + fix8LenVar + creditFix1LenVar);
    for (var i = 0; i < msgBytes; i++) {
        credit1Uncertainty.in[i] <== in[i];
    }
    signal creditFix2Array[creditFix2Len + creditFix2LenVar];
    for (var i = 0; i < creditFix2Len + creditFix2LenVar; i++) {
        creditFix2Array[i] <== credit1Uncertainty.out[i];
    }


    component creditFix2Regex = CreditFix2Regex();
    creditFix2Regex.encoded <== encodedLen;
    for (var i = 0; i < creditFix2Len + creditFix2LenVar; i++) {
        creditFix2Regex.in[i] <== creditFix2Array[0 + i];
    }


    component credit2Uncertainty = UncertaintyExtraction(credit2Range, creditFix3Len + creditFix3LenVar, partMin + fix1Len + paymentMin + fix2Len + photoMin + fix3Len + nameMin + fix4Len + usernamePartMin + fix5Len + firstAmountMin + fix6Len + messageMin + fix7Len + amountMin + fix8Len + creditFix1Len + credit1Min + creditFix2Len + credit2Min, msgBytes);
    credit2Uncertainty.indicatorLen <== credit2Len - credit2Min + credit1Len - credit1Min + amountLen - amountMin + messageLen - messageMin + firstAmountLen - firstAmountMin + usernamePartLen - usernamePartMin + nameLen - nameMin + photoLen - photoMin + paymentLen - paymentMin + partLen - partMin + encodedLen * (fix1LenVar + fix2LenVar + fix3LenVar + fix4LenVar + fix5LenVar + fix6LenVar + fix7LenVar + fix8LenVar + creditFix1LenVar + creditFix2LenVar);
    for (var i = 0; i < msgBytes; i++) {
        credit2Uncertainty.in[i] <== in[i];
    }
    signal creditFix3Array[creditFix3Len + creditFix3LenVar];
    for (var i = 0; i < creditFix3Len + creditFix3LenVar; i++) {
        creditFix3Array[i] <== credit2Uncertainty.out[i];
    }


    component creditFix3Regex = CreditFix3Regex();
    creditFix3Regex.encoded <== encodedLen;
    for (var i = 0; i < creditFix3Len + creditFix3LenVar; i++) {
        creditFix3Regex.in[i] <== creditFix3Array[0 + i];
    }

    component fix9Uncertainty = UncertaintyExtraction(credit2Range + creditFix3LenVar, fix9Len + fix9LenVar, partMin + fix1Len + paymentMin + fix2Len + photoMin + fix3Len + nameMin + fix4Len + usernamePartMin + fix5Len + firstAmountMin + fix6Len + messageMin + fix7Len + amountMin + fix8Len + creditFix1Len + credit1Min + creditFix2Len + credit2Min + creditFix3Len, msgBytes);
    fix9Uncertainty.indicatorLen <== credit2Len - credit2Min + credit1Len - credit1Min + amountLen - amountMin + messageLen - messageMin + firstAmountLen - firstAmountMin + usernamePartLen - usernamePartMin + nameLen - nameMin + photoLen - photoMin + paymentLen - paymentMin + partLen - partMin + encodedLen * (fix1LenVar + fix2LenVar + fix3LenVar + fix4LenVar + fix5LenVar + fix6LenVar + fix7LenVar + fix8LenVar + creditFix1LenVar + creditFix2LenVar + creditFix3LenVar);
    for (var i = 0; i < msgBytes; i++) {
    ...
    component methodUncertainty = UncertaintyExtraction(methodRange, fix10Len + fix10LenVar, partMin + fix1Len + paymentMin + fix2Len + photoMin + fix3Len + nameMin + fix4Len + usernamePartMin + fix5Len + firstAmountMin + fix6Len + messageMin + fix7Len + amountMin + fix8Len + fix9Len + methodMin + creditFix1Len + credit1Min + creditFix2Len + credit2Min + creditFix3Len, msgBytes);
    methodUncertainty.indicatorLen <== methodLen - methodMin + credit2Len - credit2Min + credit1Len - credit1Min + amountLen - amountMin + messageLen - messageMin + firstAmountLen - firstAmountMin + usernamePartLen - usernamePartMin + nameLen - nameMin + photoLen - photoMin + paymentLen - paymentMin + partLen - partMin + encodedLen * (fix1LenVar + fix2LenVar + fix3LenVar + fix4LenVar + fix5LenVar + fix6LenVar + fix7LenVar + fix8LenVar + fix9LenVar + creditFix1LenVar + creditFix2LenVar + creditFix3LenVar);
    for (var i = 0; i < msgBytes; i++) {
